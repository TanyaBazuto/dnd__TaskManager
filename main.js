...
function initData() {
  return {
    todo: ["Welcome to Task Manager!", "This is a card", "Click on the button below to add a new task."],
    inProgress: ["Make as many tasks as you need!", "As I was saying, this problem here will take some of you many months to solve", "Finished with a task? Drop it in Done"],
    done: ["Well done!"]
  };
}
;// ./src/js/Storage.js
...
elTasks.addEventListener("mousedown", event => {
  ...
  if (event.target.classList.contains("add-card")) {
    ...
    elInput.value = "";
    event.target.closest(".col-tasks").querySelector(".add-card").classList.remove("hidden");
    event.target.parentNode.classList.add("hidden");
    toObjectTasks();
